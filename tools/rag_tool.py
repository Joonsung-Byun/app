...
def search_facilities(
    ...
    logger.info(f"region: {region}, is_indoor: {is_indoor}, child_age: {child_age}, k: {k}")
    logger.info(f"{'='*50}")
    ...
    try:
        # 전체 데이터 가져오기 (벡터 검색 없이)
        logger.info("ChromaDB에서 전체 데이터 가져오는 중...")
        ...
        logger.info(f"✅ 전체 데이터: {len(all_data['ids'])}개")
